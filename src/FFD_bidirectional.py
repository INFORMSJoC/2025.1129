...
def model_bidirectionalFFD(m: int, L: int):
    model = gp.Model("bidirectionalFFD")
    ...
    gamma = model.addVar(lb=1.0, ub=7.0, name="C")
    model.addConstr(gamma <= gamma_FFD)
    model.addConstr(gamma <= gamma_rFFD)
...
if __name__ == "__main__":
    # Reproduce the third column of Table 3 of the paper.
    for m, L in [
        (2,6),
        (3,8),
        (4,10),
    ]:
        model = model_bidirectionalFFD(m=m, L=L)
        model.optimize()
